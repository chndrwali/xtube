...
export const playlistsRouter = createTRPCRouter({
  ...
  getVideos: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .where(and(eq(videos.visibility, 'public'), cursor ? or(lt(videos.updatedAt, cursor.updatedAt), and(eq(videos.updatedAt, cursor.updatedAt), lt(videos.id, cursor.id))) : undefined))
  ...
  addVideo: protectedProcedure.input(z.object({ playlistId: z.string().uuid(), videoId: z.string().uuid() })).mutation(async ({ input, ctx }) => {
    ...
    const [createdPlaylistVideo] = await db.insert(playlistVideos).values({ videoId, playlistId }).returning();
  ...
  getManyForVideo: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .where(and(eq(playlists.userId, userId), cursor ? or(lt(playlists.updatedAt, cursor.updatedAt), and(eq(playlists.updatedAt, cursor.updatedAt), lt(playlists.id, cursor.id))) : undefined))
  ...
  getLiked: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .where(and(eq(videos.visibility, 'public'), cursor ? or(lt(viewerVideoReactions.likedAt, cursor.likedAt), and(eq(viewerVideoReactions.likedAt, cursor.likedAt), lt(videos.id, cursor.id))) : undefined))
  ...
  getHistory: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .where(and(eq(videos.visibility, 'public'), cursor ? or(lt(viewerVideoViews.viewedAt, cursor.viewedAt), and(eq(viewerVideoViews.viewedAt, cursor.viewedAt), lt(videos.id, cursor.id))) : undefined))
